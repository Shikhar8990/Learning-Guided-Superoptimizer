...
(define enumerator%
  (class object%
    ...
    ;; bulk pruning model
    (define (filter-with-bulk-pruning opcode-pool pSize)
      opcode-pool)
    ...
    (define (generate-inst live-in live-out prune-in prune-out pSize
      ...
      (set! opcode-pool (filter-with-pruning-info opcode-pool prune-in prune-out
                                                  #:no-args no-args
                                                  #:try-cmp try-cmp))
      ;;here is the drill get recommendations only when serching and not
      ;;exploring
      (when (and get-rec #t)
        ...
        (define new-opcode-pool (filter-with-bulk-pruning opcode-pool pSize))
