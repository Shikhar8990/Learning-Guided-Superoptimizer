...
(define forwardbackward%
  (class decomposer%
	...
	(map (lambda (x) (mask-in (send machine progstate->vector x) live2-list #:keep-flag try-cmp)) states2))
      ...
      (pretty-display "!!!!!!!DISABLED INVERSE BEHAVIOUR GENERATION!!!!!!")
      (pretty-display "!!!!!!!DISABLED BW SEARCH!!!!!!")
      (fprintf (current-output-port) "Here3 \n")
      ...
      (define (check-final p)
        ...
        (fprintf (current-output-port) "Final Program Candidate \n")
        ;(send printer print-syntax-inst (send printer decode-inst final-program))
        ;(pretty-display final-program)
        (send printer print-syntax (send printer decode final-program))
      ...
      (define (check-eqv progs progs-bw my-inst my-ce-count)
        ...
        (fprintf (current-output-port) "Check Equivalent Progs \n")
        (pretty-display (hash? progs))
        ...
        (define (inner-behaviors p)
          ...
          (fprintf (current-output-port) "Inner Behaviours \n")
          (send printer print-syntax (send printer decode p))
          (fprintf (current-output-port) "---------------------- \n")
        ...
        ;;ohkay here is the deal, in this section, I an know all the programs
        ;;that are being explored to add to the new instruction, given this
        ;;instruction and the enumerated programs, select a program likely to
        ;;be there
        (fprintf (current-output-port) "Set of forward programs\n")
        ;;(system "rm -f exPrograms")
        (define fport (open-output-file "exPrograms" #:exists 'append))
        (write "st-wave" fport)
        (newline fport)
        (for* ([pr h1])
          (write "stprog" fport)
          (newline fport)
          (send printer print-syntax (send printer decode pr))
          (send printer print-syntax-file fport (send printer decode pr))
          ;(write (send printer print-syntax (send printer decode pr)) out)
          ;(write (send printer decode pr) out)
          (write "endprog" fport)
          (newline fport)
          (newline))
          (newline)
        (pretty-display h1)
        (write "end-wave" fport)
        (newline fport)
                    ...
                    (for ([pair (hash->list prev)])
                         (let* ([live-list (car pair)]
                           ...
                           (when rep
                                 ...
                                 (fprintf (current-output-port) "Here1 \n")
                                 (define iterator (send enum generate-inst 
                                                        #f live-list #f prune 0
                                                        #:try-cmp try-cmp
      ...
      (define (main-loop size)
        (when
         ...
         (for* ([key1 keys]
               ...
               (let* ([my-hash2 (hash-ref (vector-ref classes-bw step-bw) live2)]
                 ...
                 (when
                  pass
                  (fprintf (current-output-port) "Here2 \n")
        ...
        (when (and (< size size-to) (or (not cost) (> cost (add1 size))))
              ...
              (for ([pair (hash->list prev-classes)])
                   (when (> (- (current-seconds) start-time) time-limit)
                         (yield "timeout"))
                   (fprintf (current-output-port) "Here4 \n")
                   (let* ([key (car pair)]
