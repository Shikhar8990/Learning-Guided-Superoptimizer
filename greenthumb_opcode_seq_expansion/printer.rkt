...
(define printer%
  (class object%
    ...
    (public encode decode 
            ...
            print-syntax-file print-syntax-inst-file
    ...
    (define (print-syntax-inst-file fport x [indent ""])
      (write 
       (format "~a~a ~a" 
               indent (inst-op x)
               (string-join (vector->list (inst-args x)) ", ")) fport)
      (newline fport))
    ...
    (define (print-syntax-file fport x [indent ""])
      (define (inc ind) (string-append ind " "))
      (define (f x indent)
        (cond
         [(inst? x) (print-syntax-inst-file fport x indent)]
         
         [(or (list? x) (vector? x))
          (for ([i x]) (f i indent))]
         
         [else
          (write (format "~a~a" indent x) fport)
          (newline fport)]))
      (f x indent))
