...
(define enumerator%
  (class object%
    ...
    ;; bulk pruning model
    (define (filter-with-bulk-pruning opcode-pool)
      opcode-pool)
    ...
    (define (generate-inst live-in live-out prune-in prune-out
      ...
      ;;pruning if the prune flag is true otherwise restrict
      ...
      (when (equal? prune #t)
        ...
        (define new-opcode-pool (filter-with-bulk-pruning opcode-pool prune))
